% Adaptive output tracking for a one-dimensional heat equation. 
% Matlab code for the simulation example in the manuscript 
% "Nonautonomous Controllers and Output Regulation of Unknown Harmonic 
% Signals for Regular Linear Systems" by Lassi Paunonen and Sepideh Afshar, 
% IEEE Transactions on Automatic Control, 2024.
%
% Main simulation file. 
% 
% The simulation file includes "Constr1DHeatModel", "Plot1DHeatSurf" and
% "PlotEigs" from the RORPack Matlab toolbox
% (https://github.com/lassipau/rorpack-matlab/)
% 
% Copyright (C) 2023 by Lassi Paunonen.
...
% Atilde= A + 0.5*eye(N);
Atilde= A + 1.5*diag(sin(pi*spgrid));
% Atilde = A;
...
% Multiple frequency case, Carnevale-Astolfi 2ACC 008, Proposition 2:
...
% In order to estimate frequencies from a vector-valued signals, multiple 
% outputs, define y_aux(t) as a random linear combination of the original components.
if dimY > 1
    rgen = rng(1,'twister');
    % rand('seed',0);
...
%% Construction of the frequency estimator 
...
% The function R(xi) from Carnevale-Astolfi 2008
...
% Allow at most 'no_K_update_max' consequtive steps without updating K(t_j)
no_K_update_max = 0;
no_K_update_count = 0;
...
for ind = 1:Nsteps
    ...
    % Variable 'true' if all frequencies are larger than equal to 'eps_f'
    % and smaller or equal to 'M_f'
    freq_cond_sizes = all(est_freqs>=eps_f) && all(est_freqs<=M_f);
    
    % Variable 'true' if there are no overlapping frequencies (within the
    % tolerance 'eps_f')
    freq_cond_overlap = (length(real(est_freqs))==length(uniquetol(real(est_freqs),eps_f,'DataScale',1)));
    ...
    % 'eps_omega')
    freq_cond_TZ = ~any(ismembertol([real(est_freqs(:));-real(est_freqs(:))],TZ_imag,eps_f,'DataScale',1));
    ...
    % internal model. 
    if freq_cond_sizes && freq_cond_overlap && freq_cond_TZ && freq_cond_real
        IM_freqs = est_freqs;
        ...
        if ind==1 || max(real(eig(full(Ae0(IM_freqs)+Be0(IM_freqs)*K))))>-CL_stabmarg_par || no_K_update_count>=no_K_update_max
            ...
            K1 = -lqr(G1(IM_freqs)+r_stab*eye(dimZ0),B1j,10*eye(dimZ0),0.01*eye(dimU),zeros(dimZ0,dimU));
        ...
        elseif ind~=1
            ...
            no_K_update_count = no_K_update_count+1;
        end
    elseif ind==1
        ...
        if any(est_freqs>M_f), fprintf('A frequency estimate too large\n'),end
        if any(est_freqs<eps_f), fprintf('A frequency estimate too close to zero\n'),end
...
%     % Alternative: Frequency estimation with FFT:
...
cla
hold on
plot(tt_full,yref(tt_full),'color',[0.8500, 0.3250, 0.0980],'Linewidth',2)
plot(tt_full,yy_full,'color',[0, 0.4470, 0.7410],'Linewidth',2)
title('Regulated output $y(t)$.','Interpreter','Latex','FontSize',18)
xlim([0,Nsteps*timestep])
grid on
set(gcf,'Position',[915,871,827,367]);
saveas(gcf,'AdaORPsim_output.png')
figure(2)
plot(tt_full,freqs_full,'Linewidth',2);
title('Estimated frequencies.','Interpreter','Latex','FontSize',18)
xlim([0,Nsteps*timestep])
grid on
set(gcf,'Position',[915,871,827,367]);
saveas(gcf,'AdaORPsim_frequencies.png')

figure(3)
plot(tt_full,yaux_full,'Linewidth',2);
title('The residual output $y_{aux}(t)$.','Interpreter','Latex','FontSize',18)
xlim([0,Nsteps*timestep])
figure(4)
plot(tt_full,sqrt(sum(abs(yy_full-yref(tt_full)).^2,1)),'Linewidth',2);
% axis([tt_full(1),tt_full(end),0,0.2])
title('The norm of the regulation error $\|e(t)\|$.','Interpreter','Latex','FontSize',18)
xlim([0,Nsteps*timestep])

figure(5)
surf_tskip = 1;
surf_xiskip = 3;
Plot1DHeatSurf(xe_full(1:surf_xiskip:N,1:surf_tskip:end),spgrid(1:surf_xiskip:N),tt_full(1:surf_tskip:end),BCtype)
set(gcf,'Position',[656,388,1419,625]);
saveas(gcf,'AdaORPsim_state.png')

figure(6)
subplot(2,1,1)
plot(tt_full,freqs_full,'Linewidth',2);
title('Estimated frequencies $\hat{\omega}_k(t)$.','Interpreter','Latex','FontSize',18)
% xlabel('t','Interpreter','Latex')
xlim([0,Nsteps*timestep])
xticks((0:Nsteps)*timestep)
...
xlim([0,Nsteps*timestep])
xticks((0:Nsteps)*timestep)
set(gcf,'Position',[612,598,779,480]);
saveas(gcf,'AdaORPsim_combined.png')
